    ...
    try:
        ...
        async with streamablehttp_client(mcp_url, headers, timeout=120, terminate_on_close=False) as (
            ...
            async with ClientSession(read_stream, write_stream) as session:
                ...
                # Add timeout for list_tools - increase timeout
                try:
                    tool_result = await asyncio.wait_for(session.list_tools(), timeout=60)  # 30초에서 60초로 증가
                    print(f"8. session.list_tools() 성공!")
                    print(f"\nAvailable tools: {len(tool_result.tools)}")
                    for tool in tool_result.tools:
                        print(f"  - {tool.name}: {tool.description[:100]}...")
                except asyncio.TimeoutError:
                    print("❌ session.list_tools() 타임아웃 (60초)")
                    print("list_tools를 건너뛰고 직접 툴 호출을 시도합니다...")
                    # Continue without list_tools
