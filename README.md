...
생성된 policy의 이름은 BedrockAgentCoreMCPRoleFor에 project name을 합한 형태입니다. config.json.sample을 config.json으로 변경한 후에 필요한 값들을 채워줍니다. 이후 아래와 같이 IAM policy를 생성합니다. 상세코드는 [create_iam_policies.py](./runtime/kb-retriever/create_iam_policies.py)을 참조합니다. 
...
이후 아래와 설치를 준비합니다. 아래 명령어로 [Dockerfile](./mcp_server_cli/Dockerfile)과 [.bedrock_agentcore.yaml](./mcp_server_cli/.bedrock_agentcore.yaml)이 생성됩니다. 
